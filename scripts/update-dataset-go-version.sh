...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      echo "Script for updating dataset-go version in configs"
      exit 0
      ;;
    *)
...
function usage() {
  ...
  echo "Example: $0 -f v0.18.0 -t v0.20.0"
}
...
log "Updating dataset-go versions in files from ${FROM_VERSION} to ${TO_VERSION}"
